...
class Active_screen(QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        # Initialize circular timer widget safely
        try:
            self.init_circular_timer_widget(self.centralwidget)
            # Raise circular timer widget after successful initialization
            if hasattr(self, 'circular_timer_widget') and self.circular_timer_widget:
                try:
                    # Validate widget is still alive before accessing
                    self.circular_timer_widget.isVisible()
                    self.circular_timer_widget.raise_()
                except RuntimeError:
                    logger.debug("Circular timer widget was deleted during setup, will reinitialize")
    ...
    def update_timer(self):
        ...
        if self.countdown_time == 0:
            ...
            # Update circular timer to show completion (only if backend exists)
            if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
                self.circular_timer_backend.timerValueChanged.emit(0, 100)
            

        # Update circular timer with current countdown time (only if backend exists)
        if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
    ...
    def start_game(self):
        ...
        # Start the circular timer (only if backend exists)
        if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
    ...
    def stop_game(self):
        ...
        # Stop the circular timer (only if backend exists)
        if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
            self.circular_timer_backend.stop_countdown()
    ...
    def restart_game(self):
        ...
        # Restart the circular timer (only if backend exists)
        if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
    ...
    def closeEvent(self, event):
        ...
        # Cleanup circular timer widget (but preserve backend for reuse)
        if hasattr(self, 'circular_timer_widget') and self.circular_timer_widget:
            try:
                # Test if widget is still valid before accessing it
                try:
                    self.circular_timer_widget.isVisible()
                    # Widget is valid, clean it up
                    self.circular_timer_widget.deleteLater()
                    logger.debug(" Circular timer widget cleaned up")
                except RuntimeError:
                    # Widget was already deleted
                    logger.debug(" Circular timer widget was already deleted")
                finally:
                    self.circular_timer_widget = None
                    
                # Mark as not initialized for next time
                self.circular_timer_initialized = False
                    
            except Exception as e:
                logger.warning(f"️  Error cleaning up circular timer widget: {e}")
                self.circular_timer_widget = None
                self.circular_timer_initialized = False
        
        # Note: Keep circular timer backend alive for reuse - don't delete it
        if hasattr(self, 'circular_timer_backend') and self.circular_timer_backend:
            try:
                self.circular_timer_backend.stop_countdown()
                logger.debug(" Circular timer backend stopped (preserved for reuse)")
            except Exception as e:
                logger.warning(f"️  Error stopping circular timer backend: {e}")
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.ui_active, 'reset_circular_timer'):
                    ...
                    logger.debug(" Circular timer reset completed")
